...
#特别注意红色是两个范围的
COLOR_RANGES = {
    "red": [0, 153, 18, 11, 255, 255, 150, 30, 50, 180, 255, 255],  # red目标，配置方法：在实际环境下调整H值
    "blue": [86, 192, 61, 143, 255, 255],  # blue目标，配置方法：在实际环境下调整H值
...
#这个还有待测量
SAFE_ZONE_CONFIG = {
    "aspect_ratio_min": 1.0,      # 最小长宽比
    "aspect_ratio_max": 6,      # 最大长宽比
    "min_area": 100,            # 最小面积（像素）
    "max_area": 50000,          # 最大面积（像素）
    "base_area": 1000,         # 基础面积（像素²），用于距离计算
    "base_distance": 520.0,     # 基础距离（毫米），用于距离计算
}